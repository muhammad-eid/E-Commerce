...
class Category{
    static getCategorys = async function(req, res){
        try{
    ...
    static getCategory = async function(req, res){
        try{
            const profileId = req.params.id
            if(profileId===undefined || profileId==req.user._id){
                const user = await User.findById(req.user._id)//user arlady exsist in req if it is auth
    ...
    static addCategory = async function(req, res){
        try{
    ...
    static editCategory = function(req, res){
        res.send(apiRes(true, {"data":"editAdresses"}, "message"))
    }
    static removeCategory = function(req, res){
